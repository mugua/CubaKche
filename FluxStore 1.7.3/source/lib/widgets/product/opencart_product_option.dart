              ...
              onChanged: (item) {
                setState(() {
                  valueTxt = item["name"].toString() +
                      " (${item["price_prefix"]}${Tools.getCurrecyFormatted(item["price"])})";
                });
                widget.onChanged({
                      ...
                      return ListTile(
                          ...
                          title: Text(
                            option["name"].toString() +
                                " (${option["price_prefix"]}${Tools.getCurrecyFormatted(option["price"])})",
...
class InputDateTime extends StatelessWidget {
  ...
  Future _selectDate(context) async {
    DateTime picked = await showDatePicker(
        ...
        firstDate:
            DateTime.now().add(Duration(days: 1)).subtract(Duration(hours: 1)),
  ...
  Future _selectDateTime(context) async {
    DatePicker.showDatePicker(context,
        pickerMode: DateTimePickerMode.datetime, dateFormat: "dd-MM-yyyy HH:mm",
        onConfirm: (DateTime dateTime, List<int> selectedIndex) {
      onChanged(DateFormat.yMd().add_jm().format(dateTime).toString());
    });
  }
}
