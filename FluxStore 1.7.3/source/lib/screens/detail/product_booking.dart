...
class _StateProductBooking extends State<ProductBooking> {
  ...
  DateTime supportDate(String unit, int value) {
    ...
    return DateTime.fromMillisecondsSinceEpoch(_milliSecond + _duration.inMilliseconds);
  }

  Future<void> getBookingData() async {
    var response = await http
        .get('${Config().url}/wp-json/wc-bookings/v1/products/${widget.product.id}');
  ...
  Future<void> datePicker() async {
    final _date = await showDatePicker(
        context: context, initialDate: minDate, firstDate: minDate, lastDate: maxDate);
  ...
  Future<bool> checkField() async {
    var response = await http.post('${Config().url}/wp-admin/admin-ajax.php', headers: {
          ...
          child: Row(
            children: <Widget>[
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  Text(S.of(context).duration, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
                  SizedBox(
                    height: 2,
                  ),
                  Expanded(
